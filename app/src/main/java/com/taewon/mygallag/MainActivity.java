...
public class MainActivity extends AppCompatActivity {
    ...
    public static float effectVolum;
    ...
    private void changeBgMusic(){
        bgMusic = MediaPlayer.create(this, bgMusicList.get(bgMusincIndex));  //음악하나 가져와 만들기
        bgMusic.start();
        bgMusincIndex++;
        bgMusincIndex = bgMusincIndex % bgMusicList.size();  //음악 개수만큼만 바뀌게  하기 위해
    }
    ...
    public static void effectSound(int flag) {
        effectSound.play(effectSoundList.get(flag), effectVolum, effectVolum,0,0,1.0f);
        //Soundpool 실행
    }
    ...
    private void setBtnBehavior() {
        ...
        fireBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) { spaceInvadersView.getPlayer().fire(); }
        });
